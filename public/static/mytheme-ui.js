...
*/var MyTheme = {
    Browser: {
        ...
        useragent: function () {
            ...
            return {
                ...
                iPad: -1 < a.indexOf("iPad"), trident: -1 < a.indexOf("Trident"),
    ...
    Ajax: function (url, type, dataType, data, sfun, efun, cfun) {
        ...
        $.ajax({
            ...
            error: function (XHR, textStatus, errorThrown) { if (efun) efun(XHR, textStatus, errorThrown); },
            success: function (data) { sfun(data); }, complete: function (XHR, TS) { if (cfun) cfun(XHR, TS); }
        })
    },
    Mobile: {
        Nav: {
            ...
            Set: function (id, align) {
                ...
                id.flickity({ cellAlign: align, freeScroll: true, contain: true, prevNextButtons: false, pageDots: false, percentPosition: true, initialIndex: $index });
            }
        },
        Mshare: function () {
    ...
    Images: {
        ...
        Flickity: {
            ...
            Set: function (id, align, dots, next, play) {
                ...
                next = next || false;
                play = play || false;
                id.flickity({ cellAlign: align, wrapAround: true, contain: true, pageDots: dots, autoPlay: play, percentPosition: true, prevNextButtons: next });
            }
        }
    }, Link: {
        Copy: {
            ...
            $(".myui-short").each(function () {
                ...
                if (myui.short == 1) {
                    $.ajax({
                        type: 'GET', url: myui.shortapi + encodeURIComponent(shortUrl), dataType: 'jsonp', success: function (r) {
    ...
    Layer: {
        Img: function (title, src, text) {
            layer.open({ type: 1, title: title, skin: 'layui-layer-rim', content: '<div class="col-pd"><p><img src="' + src + '" width="240" /></p><p class="text-center">' + text + '</p></div>' });
        },
        Html: function (title, html) {
            layer.open({ type: 1, title: title, skin: 'layui-layer-rim', content: '<div class="col-pd">' + html + '</div>' });
        },
        Div: function (id) {
            layer.open({ type: 1, title: false, skin: 'layui-layer-rim', content: $(id) });
        },
        Popbody: function (name, html, day, wide, high) {
            var pop_is = MyTheme.Cookie.Get(name);
            var html = $(html).html(); if (!pop_is) {
                layer.open({
                    type: 1, title: false, content: html, area: [wide + 'px', high + 'px'],
                    cancel: function () { MyTheme.Cookie.Set(name, 1, day); }
                });
            }
        }
    },
    Other: {
        ...
        Skin: function () {
            ...
            if (color) { $("link[name='default']").attr({ href: color }); }
        }, Sort: function () {
                ...
                $(this).on("click", function (e) {
                    ...
                    $(this).parent().parent().parent().find(".sort-list").each(function () {
                        ...
                        for (let i = 0, j = playlist.length - 1; i < j;) {
                            ...
                            playlist.eq(i).replaceWith(r); ++i; --j;
                        }
                    });
                });
            });
                ...
                if (!MyTheme.Browser.useragent.mobile) {
                    $(window).scroll(function () {
                        if ($(window).scrollTop() > window.outerHeight) {
                            ...
                            $(".player-fixed-off").show(); $(".close-box").hide();
        ...
        Roll: function (obj, higt, time) {
            setInterval(function () {
                $(obj).find("ul").animate({ marginTop: higt, }, time, function () {
        ...
        Xunlei: function () {
            $.getScript(myui.thunderurl, function () {
                $(".common_down").on("click", function () {
                    ...
                    thunderLink.newTask({ downloadDir: '下载目录', tasks: [{ name: filename, url: url, size: 0 }] });
                });
                    ...
                    if (checked.length < 1) {
                        ...
                        for (let i = 0; i < links.length; ++i) {
                            if (selectbox.eq(i).is(':checked')) {
                                var task = { url: links.eq(i).val(), size: 0 };
                                tasks.push(task);
                            }
                        }
                        thunderLink.newTask({ downloadDir: '下载目录', installFile: '', taskGroupName: '下载文件', tasks: tasks, excludePath: '' });
                    }
                });
                ...
                if ($(".thunderkk").length) {
                    $(".thunderkk").on("click", function () {
                        ...
                        var url = link.eq(0).val(); kkPlay(url, "");
                    });
                    var kkDapCtrl = null; function kkGetDapCtrl() {
                        if (null == kkDapCtrl) {
                            try {
                                    ...
                                    for (var bpi = 0; bpi < browserPlugins.length; bpi++) {
                                        try {
                                            if (browserPlugins[bpi].name.indexOf('Thunder DapCtrl') != -1) {
                                                ...
                                                e.id = "dapctrl_history"; e.type = "application/x-thunder-dapctrl";
                                                e.width = 0; e.height = 0;
                                                document.body.appendChild(e); break;
                                            }
                                        } catch (e) { }
                                    }
