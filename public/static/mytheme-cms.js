...
*/var Myui = {
    Comment: {
        Init: function () {
            ...
            $('body').on('click', '.my_comment_reply', function (e) {
                ...
                if ($that.attr("data-id")) {
                    ...
                    var oo = $(html); oo.addClass('comment_reply_form');
        ...
        Show: function ($page) {
            MyTheme.Ajax(maccms.path + '/index.php/comment/ajax.html?rid=' + $('.myui_comment').attr('data-id') + '&mid=' + $('.myui_comment').attr('data-mid') + '&page=' + $page, 'get', 'json', '',
                function (r) {
                    $(".myui_comment").html(r);
                },
                function () {
    ...
    Gbook: {
        Init: function () {
            $('body').on('click', '.gbook_submit', function (e) {
                if ($(".gbook_data").val() == '') { layer.msg("请输入留言内容"); return false; }
                Myui.Gbook.Submit();
            });
        }, Submit: function () {
            MyTheme.Ajax(maccms.path + '/index.php/gbook/saveData', 'post', 'json', $("#myform").serialize(), function (r) {
                if (r.code == 1) {
                    layer.msg(r.msg, { anim: 5 }, function () { location.reload(); });
                } else {
    ...
    Score: function () {
        ...
        $("ul.rating li").each(function (i) {
            ...
            var num = $(this).index();
            var n = num + 1; $(this).click(function () {
                if (hadpingfen > 0) {
                    ...
                    $.getJSON(maccms.path + '/index.php/ajax/score?mid=' + $('#rating').attr('data-mid') + '&id=' + $('#rating').attr('data-id') + '&score=' + ($(this).attr('val') * 2),
                        function (r) {
                            if (parseInt(r.code) == 1) {
                                layer.msg(r.msg); hadpingfen = 1;
                            } else {
                                hadpingfen = 1; layer.msg(r.msg);
                            }
                        });
                }
            }).hover(function () {
            ...
            $('.search_wd').autocomplete(maccms.path + '/index.php/ajax/suggest?mid=1',
                {
                    ...
                    formatResult: function (row, i, max) { return row.text; }
    ...
    User: {
        BuyPopedom: function (o) {
            var $that = $(o); if ($that.attr("data-id")) {
                if (confirm('您确认购买此条数据访问权限吗？')) {
                    MyTheme.Ajax(maccms.path + '/index.php/user/ajax_buy_popedom.html?id=' + $that.attr("data-id") + '&mid=' + $that.attr("data-mid") + '&sid=' + $that.attr("data-sid") + '&nid=' + $that.attr("data-nid") + '&type=' + $that.attr("data-type"),
                        'get', 'json', '', function (r) {
                            $that.addClass('disabled');
                            layer.msg($r.msg);
                            if (r.code == 1) {
